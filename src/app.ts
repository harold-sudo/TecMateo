...
import { required, isEmail, isPositiveInteger } from "./utils/validators";
...
function bindEvents(){
  ...
  const dashEst = q<HTMLFormElement>('#dash-est-form');
  const dashCur = q<HTMLFormElement>('#dash-cur-form');
  if(dashEst){
    dashEst.addEventListener('submit', (e)=>{
      ...
      const correo = (fd.get('correo') as string||'').trim();
      try{
        if (!required(nombre) || !required(correo)) throw new Error('Todos los campos son requeridos');
        if (!isEmail(correo)) throw new Error('Email inválido');
        // defaults for quick create
        estudianteSvc.create({ nombre, correo, edad: 18, carrera: 'General', estado: 'activo' });
        showMessage('Estudiante creado');
        dashEst.reset(); refreshAll();
      }catch(err:any){ showMessage(err.message||String(err), true); }
    });
  }
  if(dashCur){
    dashCur.addEventListener('submit', (e)=>{
      ...
      const sigla = (fd.get('sigla') as string||'').trim();
      try{
        if (!required(nombre) || !required(sigla)) throw new Error('Todos los campos son requeridos');
        // defaults for quick create
        cursoSvc.create({ nombre, sigla, docente: 'TBD', cupoMaximo: 30, estado: 'disponible' });
        showMessage('Curso creado');
        dashCur.reset(); refreshAll();
      }catch(err:any){ showMessage(err.message||String(err), true); }
    });
  }

  q('#est-cancel')!.addEventListener('click', ()=>{ editingEstId = null; (q('#est-form') as HTMLFormElement).reset(); q('#est-cancel')!.style.display='none'; });
  q('#cur-cancel')!.addEventListener('click', ()=>{ editingCurId = null; (q('#cur-form') as HTMLFormElement).reset(); q('#cur-cancel')!.style.display='none'; });
...
function refreshAll(){
  renderLists(); fillInsSelects(); fillRelationsSelects(); renderStats();
}

function fillInsSelects(){
  const selEst = q<HTMLSelectElement>('#insc-est')!; selEst.innerHTML='';
  ...
  const selCur = q<HTMLSelectElement>('#insc-curso')!; selCur.innerHTML='';
...
function fillRelationsSelects(){
  const relE = q<HTMLSelectElement>('#rel-est')!; relE.innerHTML='<option value="">--seleccione--</option>';
  ...
  const relC = q<HTMLSelectElement>('#rel-cur')!; relC.innerHTML='<option value="">--seleccione--</option>';
...
function renderRelEst(id: number){
  const ul = q<HTMLUListElement>('#rel-est-list')!; ul.innerHTML='';
  if (!id) return;
...
function renderRelCur(id: number){
  const ul = q<HTMLUListElement>('#rel-cur-list')!; ul.innerHTML='';
  if (!id) return;
...
function renderLists(){
  const estContainer = q<HTMLElement>('#est-list')!; estContainer.innerHTML = '';
  const curContainer = q<HTMLElement>('#cur-list')!; curContainer.innerHTML = '';
  const insContainer = q<HTMLElement>('#insc-list')!; insContainer.innerHTML = '';
  ...
  estudianteSvc.getAll().filter(s=> (s.nombre.toLowerCase().includes(searchEst)) && (filterEst==='all' || s.estado===filterEst)).forEach(s=>{
    ...
    const btnEdit=document.createElement('button'); btnEdit.textContent='Editar'; btnEdit.addEventListener('click', ()=>{ startEditEst(s.id); });
    const btnDel=document.createElement('button'); btnDel.textContent='Eliminar'; btnDel.addEventListener('click', ()=>{ if(confirm('Eliminar estudiante?')){ estudianteSvc.delete(s.id); inscSvc.getAll().filter(i=>i.estudianteId===s.id).forEach(i=>inscSvc.delete(i.id)); refreshAll(); showMessage('Estudiante eliminado'); } });
  ...
  const tableC = document.createElement('table'); tableC.innerHTML = '<tr><th>Nombre</th><th>Sigla</th><th>Docente</th><th>Cupo</th><th>Estado</th><th>Acciones</th></tr>';
  cursoSvc.getAll().filter(c=> (c.nombre.toLowerCase().includes(searchCur) || c.sigla.toLowerCase().includes(searchCur)) && (filterCur==='all' || c.estado===filterCur)).forEach(c=>{
    ...
    const td=document.createElement('td');
    const btnEdit=document.createElement('button'); btnEdit.textContent='Editar'; btnEdit.addEventListener('click', ()=>{ startEditCur(c.id); });
    const btnDel=document.createElement('button'); btnDel.textContent='Eliminar'; btnDel.addEventListener('click', ()=>{ if(confirm('Eliminar curso?')){ cursoSvc.delete(c.id); inscSvc.getAll().filter(i=>i.cursoId===c.id).forEach(i=>inscSvc.delete(i.id)); refreshAll(); showMessage('Curso eliminado'); } });
  ...
  const tableI = document.createElement('table'); tableI.innerHTML = '<tr><th>Estudiante</th><th>Curso</th><th>Fecha</th><th>Estado</th><th>Acciones</th></tr>';
  inscSvc.getAll().forEach(i=>{
    const est = estudianteSvc.findById(i.estudianteId);
    const cur = cursoSvc.findById(i.cursoId);
    const tr=document.createElement('tr'); tr.innerHTML = `<td>${est?est.nombre:i.estudianteId}</td><td>${cur?cur.nombre:i.cursoId}</td><td>${i.fecha}</td><td>${i.estado}</td>`;
    const td=document.createElement('td');
    const btnDel=document.createElement('button'); btnDel.textContent='Eliminar'; btnDel.addEventListener('click', ()=>{ if(confirm('Eliminar inscripcion?')){ inscSvc.delete(i.id); refreshAll(); showMessage('Inscripción eliminada'); } });
